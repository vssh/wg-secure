...
CLIENT_ACCESS=$2
if [[ $CLIENT_ACCESS != $CLIENT_ACCESS_FULL && $CLIENT_ACCESS != $CLIENT_ACCESS_INTRANET && $CLIENT_ACCESS != $CLIENT_ACCESS_INTERNET  && $CLIENT_ACCESS != $CLIENT_ACCESS_CUSTOM ]]; then
    if  [[ $CLIENT_ACCESS == "" ]]; then
        CLIENT_ACCESS=$CLIENT_ACCESS_DEFAULT
    else
        echo "client access must be \"FULL\", \"INTERNET\", \"INTRANET\"  or \"CUSTOM\""
        exit 1;
    fi
fi
...
function getIpsForCustomAccess() {
    local arr=""
    local lines=(${CLIENT_CUSTOM_IP_PORT//;/ })
...
CLIENT_TEMPLATE=$(cat "${SCRIPT_PATH}/${CLIENT_TEMPLATE_FILE}")
...
SERVER_PEER_TEMPLATE=$(cat "${SCRIPT_PATH}/${SERVER_PEER_TEMPLATE_FILE}")
